...
function PlayState:enter(params)
    ...
    --2.1
    self.hasKey = false
    self.powerup = Powerup(self.hasKey)
    self.hitCount = 0
    self.minPowerupNeed = 5
    self.ballCount = 1
...
function PlayState:update(dt)
    ...
    if self.powerup:collides(self.paddle) then
        ...
        self.powerup:reset()     
    elseif self.powerup.y > self.paddle.y + 32 then
        self.powerup:reset()
    end

    if self.hitCount > self.minPowerupNeed then --and self.powerup.inPlay == false then
        self.powerup.inPlay = true
        self.powerup.dy = math.random(50, 60)
        self.hitCount = 0
        if self.minPowerupNeed < 10 then
            self.minPowerupNeed = self.minPowerupNeed + math.random(1, 3)
        end
    end
    ...
    for k, ball in pairs(self.balls) do
        ...
        for k, brick in pairs(self.bricks) do
            ...
            if brick.inPlay and ball:collides(brick) then
                if brick.isLocked and self.powerup.hasKey or not brick.isLocked then
                    -- add to score
                    ...
                    if self.score > self.recoverPoints and self.paddle.size < 4 then 
...
function PlayState:insertBalls()
    ball1 = Ball(5)
    ball2 = Ball(5)
