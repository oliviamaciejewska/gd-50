...
function Level:update(dt)
    ...
    --6.1 if space is pressed create two aliens
    if love.keyboard.wasPressed('space') and self.canSplit and self.launchMarker.launched then
        local xPos, yPos = self.launchMarker.alien.body:getPosition()
        local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()

        local alien1 = Alien(self.world, 'round', xPos, yPos, 'Player')
        alien1.body:setLinearVelocity(xVel, yVel - 45)
        table.insert(self.splitAliens, alien1)

        local alien2 = Alien(self.world, 'round', xPos, yPos, 'Player')
        alien2.body:setLinearVelocity(xVel, yVel + 45)
        table.insert(self.splitAliens, alien2)
        self.canSplit = false
    end
    --[[
    if self.launchMarker.launched then
        for k, alienSplit in pairs(self.splitAliens) do
            local xPos, yPos = alienSplit.body:getPosition()
            local xVel, yVel = alienSplit.body:getLinearVelocity()
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 5) then
                table.remove(self.splitAliens, k)
            end
        end
    end
        ]]
    for k, alienSplit in pairs(self.splitAliens) do
        local xPos, yPos = alienSplit.body:getPosition()
        local xVel, yVel = alienSplit.body:getLinearVelocity()
        if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 4) or xPos >= VIRTUAL_WIDTH then
            alienSplit.body:destroy()
            table.remove(self.splitAliens, k)
        end
    end
