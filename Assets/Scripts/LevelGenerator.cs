		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				if (mapData[z, x]) {
					...
					//9.1 ensure character placed on floor
					CreateChildPrefab(floorPrefab, floorParent, x, 0, z);
				}
				else
				{
					//9.1 create a random chance of adding a floor and ensuring only 3 added
					int holeChance = (int)(Random.Range(1, 20));
					if (holeChance == 1 && holeCount < 4)
					{
						holeCount ++;
					}
					else
